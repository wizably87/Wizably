...
const PLANS = [
  {
    ...
    emails: 10, features: ["10 emails/month", "5 industries", "3 tones", "Basic support", "Copy to clipboard"],
...
export default function Wizably() {
  const [page, setPage] = useState("landing"); // landing | login | dashboard | generator | history | settings | pricing
...
{"subject":"...","greeting":"...","body":"...","cta":"...","signoff":"..."}`;
  ...
  const resetGen = () => { setGStep(1); setGenEmail(null); setREmail(""); setRName(""); setIndustry(""); setPurpose(""); setTone("Professional"); setCtx(""); setGenErr(""); };
  ...
  const g = {
    ...
    purpBtn: { background: "linear-gradient(135deg,#7c3aed,#6d28d9)", color: "#fff", border: "none", padding: "10px 24px", borderRadius: "8px", fontWeight: "700", fontSize: "14px", cursor: "pointer", boxShadow: "0 4px 16px rgba(124,58,237,0.3)" },
    outlineBtn: { background: "transparent", color: "#a78bfa", border: "1px solid #3d2d6a", padding: "10px 24px", borderRadius: "8px", fontWeight: "700", fontSize: "14px", cursor: "pointer" },
    // HERO
    hero: { padding: "90px 32px 70px", maxWidth: "900px", margin: "0 auto", textAlign: "center" },
    badge: { display: "inline-block", background: "rgba(167,139,250,0.1)", border: "1px solid rgba(167,139,250,0.25)", color: "#a78bfa", padding: "6px 16px", borderRadius: "20px", fontSize: "12px", marginBottom: "24px", letterSpacing: "0.5px" },
    ...
    pCard: (c, b, active) => ({ background: b, border: `2px solid ${c}`, borderRadius: "18px", padding: "28px", position: "relative", boxShadow: active ? `0 0 30px ${c}30` : "none" }),
    ...
    authCard: { background: "#11101a", border: "1px solid #1d1a2e", borderRadius: "20px", padding: "36px 28px", width: "100%", maxWidth: "380px", boxShadow: "0 0 60px rgba(124,58,237,0.1)" },
    ...
    sideLink: (a) => ({ display: "flex", alignItems: "center", gap: "10px", padding: "11px 20px", cursor: "pointer", background: a ? "rgba(124,58,237,0.15)" : "transparent", borderRight: a ? "3px solid #7c3aed" : "3px solid transparent", color: a ? "#a78bfa" : "#6b5f8a", fontSize: "14px", fontWeight: a ? "700" : "400", transition: "all 0.15s" }),
    ...
    progFill: (pct) => ({ height: "100%", width: `${pct}%`, background: pct > 80 ? "linear-gradient(90deg,#ef4444,#dc2626)" : "linear-gradient(90deg,#7c3aed,#a855f7)", borderRadius: "4px", transition: "width 0.5s" }),
  };
    ...
    <div>
      ...
      <nav style={g.lnav}>
        ...
        <div style={g.navLinks}>
          <button style={g.navLink} onClick={() => document.getElementById("features")?.scrollIntoView({ behavior: "smooth" })}>Features</button>
      ...
      <section style={g.hero}>
        ...
        <h1 style={g.h1}>
          Har Email<br />
          <span style={g.accent}>Industry-Specific,</span><br />
          Instantly Ready.
        </h1>
      ...
      <div style={g.statsBar}>
        {[["10,000+", "Emails Generated"], ["12+", "Industries"], ["3 Sec", "Avg. Time"], ["98%", "User Rating"]].map(([n, l]) => (
          <div key={l} style={{ textAlign: "center" }}>
            <div style={g.statNum}>{n}</div>
            <div style={g.statLbl}>{l}</div>
          </div>
        ))}
      </div>

      {/* FEATURES */}
      <section id="features" style={g.section}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(220px,1fr))", gap: "20px" }}>
          {[
            ...
            <div key={s.n} style={{ ...g.fCard, textAlign: "center" }}>
              ...
              <div style={g.fTitle}>{s.t}</div>
              <div style={g.fDesc}>{s.d}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* PRICING PREVIEW */}
      <section style={{ ...g.section, borderTop: "1px solid #13111e" }}>
        <div style={g.secLabel}>Pricing</div>
        <h2 style={g.secH2}>Simple, Transparent Plans</h2>
        <div style={g.pGrid}>
          {PLANS.map(p => (
            <div key={p.id} style={g.pCard(p.color, p.bg, p.id === "pro")}>
              {p.badge && <div style={g.pBadge(p.color)}>{p.badge}</div>}
              <div style={{ fontSize: "16px", fontWeight: "800", color: p.color, marginBottom: "8px" }}>{p.name}</div>
              <div style={{ display: "flex", alignItems: "baseline", gap: "4px", marginBottom: "20px" }}>
                <span style={{ ...g.pPrice, color: p.color }}>{p.price}</span>
                <span style={g.pPer}>{p.period}</span>
              </div>
              {p.features.slice(0, 4).map(f => <div key={f} style={g.pFeat}><span style={{ color: p.color }}>✓</span>{f}</div>)}
              <button style={{ ...g.purpFull(false), background: `linear-gradient(135deg,${p.color},${p.color}cc)`, marginTop: "16px" }}
                onClick={() => setPage("login")}>
                {p.id === "free" ? "Abhi Shuru Karo →" : `${p.name} Lein →`}
              </button>
            </div>
          ))}
        </div>
      ...
      <section style={{ margin: "0 32px 72px", background: "linear-gradient(135deg,#7c3aed,#6d28d9)", borderRadius: "20px", padding: "56px 32px", textAlign: "center" }}>
        ...
        <p style={{ color: "rgba(255,255,255,0.7)", fontSize: "16px", marginBottom: "28px" }}>Koi credit card nahi. Koi signup fee nahi. Seedha try karo.</p>
        <button style={{ background: "#fff", color: "#7c3aed", border: "none", padding: "16px 40px", borderRadius: "10px", fontSize: "17px", fontWeight: "900", cursor: "pointer" }} onClick={() => setPage("login")}>
          Free Account Banao →
        </button>
      </section>
      ...
      <div style={g.authCard}>
        ...
        {authTab === "signup" && (
          <div style={{ marginBottom: "14px" }}>
            ...
            <input style={g.inp} placeholder="Full naam likho" value={authName} onChange={e => setAuthName(e.target.value)} />
          </div>
        )}
        <div style={{ marginBottom: "14px" }}>
        ...
        <div style={{ marginTop: "20px", background: "#09090f", border: "1px solid #1d1a2e", borderRadius: "10px", padding: "12px", fontSize: "12px", color: "#6b5f8a" }}>
          <div style={{ color: "#a78bfa", fontWeight: "700", marginBottom: "4px" }}>🧪 Demo ke liye:</div>
          <div>Koi bhi email + password likho → Login ho jaao</div>
        </div>
    ...
    <div style={g.appShell}>
      <aside style={g.sidebar}>
        ...
        {[
          ...
          <div key={l.key} style={g.sideLink(page === l.key)} onClick={() => { setPage(l.key); if (l.key === "generator") resetGen(); }}>
        ...
        <div style={{ padding: "16px 20px", borderTop: "1px solid #16132a" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "12px" }}>
            <div style={{ width: "32px", height: "32px", borderRadius: "50%", background: "linear-gradient(135deg,#7c3aed,#a855f7)", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: "800", fontSize: "13px" }}>{user?.avatar}</div>
            <div><div style={{ fontSize: "13px", fontWeight: "700" }}>{user?.name}</div><div style={{ fontSize: "11px", color: "#6b5f8a" }}>{plan?.name} Plan</div></div>
          </div>
  ...
  const Generator = () => (
    <AppShell>
        ...
        {[{ n: 1, l: "Recipient" }, { n: 2, l: "Settings" }, { n: 3, l: "Email" }].map((s, i) => (
          <div key={s.n} style={{ display: "flex", alignItems: "center", flex: i < 2 ? 1 : "none" }}>
            <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
              <div style={{ width: "28px", height: "28px", borderRadius: "50%", background: gStep > s.n ? "#7c3aed" : gStep === s.n ? "#7c3aed" : "#1d1a2e", border: "2px solid transparent", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "12px", fontWeight: "800", color: gStep >= s.n ? "#fff" : "#4a3d6a" }}>{gStep > s.n ? "✓" : s.n}</div>
      ...
      {gStep === 3 && genEmail && (
        <div style={{ ...g.card, animation: "fadeIn 0.3s ease" }}>
          ...
          {HISTORY.map((h, i) => (
            <div key={h.id} style={{ ...g.tRow(i), gridTemplateColumns: "1fr auto auto", gap: "14px", marginBottom: "4px" }}>
              <div>
                ...
                <div style={{ fontSize: "12px", color: "#6b5f8a" }}>{h.to} • {h.industry} • {h.purpose}</div>
              </div>
  ...
  const PricingPage = () => (
    <AppShell>
      ...
      <div style={g.pGrid}>
        {PLANS.map(p => (
          <div key={p.id} style={{ ...g.pCard(p.color, p.bg, p.id === "pro"), marginBottom: "8px" }}>
            {p.badge && <div style={g.pBadge(p.color)}>{p.badge}</div>}
  ...
  const Settings = () => (
    <AppShell>
      ...
      <p style={{ color: "#6b5f8a", fontSize: "14px", marginBottom: "24px" }}>Account aur preferences manage karo</p>
      <div style={g.settCard}>
        ...
        <div style={{ marginBottom: "14px" }}><label style={g.genLbl}>Company Name</label><input style={{ ...g.genInp, marginTop: "6px" }} placeholder="Tumhari company..." /></div>
        <button style={{ ...g.purpFull(false), width: "auto", padding: "11px 28px", marginTop: 0 }}>Save Changes</button>
      </div>
      <div style={g.settCard}>
        <div style={{ fontSize: "14px", fontWeight: "800", color: "#a78bfa", marginBottom: "18px" }}>🔑 Password Change</div>
    ...
    <div style={g.wrap}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        input::placeholder, textarea::placeholder { color: #3a2d5a; }
        select option { background: #11101a; }
        ::-webkit-scrollbar { width: 4px; } ::-webkit-scrollbar-track { background: #09090f; } ::-webkit-scrollbar-thumb { background: #2a2040; border-radius: 4px; }
      `}</style>
      ...
      {page === "pricing" && (user ? <PricingPage /> : <Landing />)}
